...
SDK_VERSION = "0.3.1"
POSTHOG_CAPTURE_TOKEN = "phc_erM3KBxu4WfepnjJ6TLT11QA0yykiCeRQdi5S4xwCR6"
    ...
    def before_request(self, now_ms: float) -> None:
        with self._lock:
            ...
            if now_ms - self.opened_at >= self.cooldown_ms:
                ...
                return
            raise OmniDatastreamError(0, {"code": "client_circuit_open", "message": "Omni Datastream client circuit breaker is open"})
...
class OmniDatastreamClient:
    ...
    def _should_retry(self, method: str, error: Exception, retry_disabled: bool, unsafe_opt_in: bool) -> tuple[bool, int | None, str]:
        ...
        if isinstance(error, OmniDatastreamError):
            status = error.status
    ...
    def _emit_retry_telemetry(
        ...
        *,
        ...
        if disabled:
            return
        capture_token = options.get("capture_token", POSTHOG_CAPTURE_TOKEN)
    ...
    def _request(
        ...
        *,
        ...
        for attempt in range(max_retries + 1):
            ...
            if remaining_ms <= 0:
                ...
                if last_error is not None:
                    raise last_error
                raise OmniDatastreamError(0, {"code": "client_retry_budget_exceeded", "message": "Omni Datastream request exceeded retry budget"})
            ...
            try:
                ...
                last_error = OmniDatastreamError(
                    status=error.code,
            ...
            if not retryable or attempt >= max_retries:
                if retryable and circuit_eligible:
                    self._circuit_breaker.record_failure(float(now()))
                raise last_error
            delay_ms = self._retry_delay_ms(attempt, last_error.retry_after_ms if isinstance(last_error, OmniDatastreamError) and status == 429 else None, retry_options)
        ...
        raise last_error or OmniDatastreamError(0, {"code": "client_request_failed", "message": "Omni Datastream request failed"})
    ...
    def portfolio_analyze(
        ...
        *,
        ...
        return self._request("POST", "/v1/portfolio/analyze", body=body, retry=retry, telemetry=telemetry)
    ...
    def portfolio_optimize(
        ...
        *,
        ...
        return self._request("POST", "/v1/portfolio/optimize", body=body, retry=retry, telemetry=telemetry)
    ...
    def portfolio_stress_test(
        ...
        *,
        ...
        return self._request("POST", "/v1/portfolio/stress-test", body=body, retry=retry, telemetry=telemetry)
...
SecApiClient = OmniDatastreamClient
SecApiError = OmniDatastreamError
